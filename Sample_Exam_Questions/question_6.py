...
def f(for_seed, n, upper_bound):
    ...
    squ_list,duplicates = list(),list()
    for i in range(n):
        for j in range(n):
            squ_list.append(square[i][j])
    squ_list = sorted(squ_list)

    for i in range(len(squ_list)):
        if squ_list[i] == squ_list[i-1]:
            duplicates.append(squ_list[i])
    i = 0 
    duplicates = list(set(duplicates))
    ordered_square = [[0] * n for i in range(n)]
    for i in range(n):
        for j in range(n):
            ordered_square[i][j] = squ_list.pop(0)

    ordered_square = list(zip(*ordered_square))
    if duplicates:
