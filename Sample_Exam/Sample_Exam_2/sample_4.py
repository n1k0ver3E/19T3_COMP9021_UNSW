...
def get_words_and_word_relationships():
    try:
        ...
        print('Could not open {dictionary_file}. Giving up...')
        sys.exit()
...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    if word_1 in lexicon and word_2 in lexicon and candidate_ladder[0] == word_1 and  candidate_ladder[-1] == word_2:
        for i in range(len(candidate_ladder)-1,1,-1):
            if candidate_ladder[i] in closest_words[candidate_ladder[i-2]]:
                return False
        for i in range(1,len(candidate_ladder)):
            if candidate_ladder[i] in closest_words[candidate_ladder[i-1]]:
                pass
            else:
                return False
        return True
